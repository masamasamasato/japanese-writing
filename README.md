...
読みにくさの原因は、たいてい個々の文ではなく全体の構成にあります。そのため、上のステップほど直したときの効果が大きくなります。手順も上から順に進めます。
...
このリポジトリの PR では、2つのワークフローが `scripts/check.py` を自動で回してコメントします。どちらもワークフロー自体は失敗させず、点数は目安として出すだけです。
